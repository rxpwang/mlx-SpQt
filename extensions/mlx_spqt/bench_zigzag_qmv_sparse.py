...
def bench_shape(M, K):
    ...
    def make_run_sparse(idx):
        def run(x):                                                                                            
            y = mlx_spqt.zigzag_qmv_sparse(x, idx, w_zz, sc_zz, b_zz, group_size=GS, bits=BITS)
            for _ in range(LOOPS - 1):                                                                         
                y = y + mlx_spqt.zigzag_qmv_sparse(x, idx, w_zz, sc_zz, b_zz, group_size=GS, bits=BITS)
            return y                                                                                           
        return run
