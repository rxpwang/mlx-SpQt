"""SpQt → MLX integrated end-to-end demo.

Runs correctness AND performance for both mx.zigzag_qmv_dense and
mx.zigzag_qmv_sparse, across 7 LLM-relevant shapes × {1.0, 0.75, 0.5, 0.25}
sparsity densities. Single command, single Python script.

Uses the integrated MLX-core APIs: mx.zigzag_qmv_* + mlx.spqt.*. For the
parallel extension-API version, see extensions/mlx_spqt/demo_spqt.py.
...
def setup_shape(M, K):                                                                                         
    ...
    w_dq_f32         = dequantize_zigzag(w_zz, sc_f32, b_f32, group_size=GS, bits=BITS)               
...
def correctness(d):                                                                                          
    ...
    for dens in DENSITIES:                                                                                     
        ...
        y = mx.zigzag_qmv_sparse(x, idx, w_zz, sc_zz, b_zz,
        ...
        def make(idx):                                                                                         
            def run():                                                                                       
                y = mx.zigzag_qmv_sparse(x, idx, w_zz, sc_zz, b_zz,                                      
                                                group_size=GS, bits=BITS)                                    
                for _ in range(LOOPS - 1):                                                                     
                    y = y + mx.zigzag_qmv_sparse(x, idx, w_zz, sc_zz, b_zz,                            
                                                        group_size=GS, bits=BITS)                               
                return y                                                                                     
            return run                                                                                         
...
def main():
    ...
    print(f"  MLX's tuned mx.quantized_matmul at certain sparsity.")                                        
    print()                                                                                                    
